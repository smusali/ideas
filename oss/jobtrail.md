# **JobTrail** — *Simple Job Application Tracking CLI*
...
JobTrail is a simple CLI tool that lets you save job postings, track your applications, and stay organized during your job search directly from your terminal. Perfect for developers and job seekers who want to keep their job hunt organized without complex applications.
...
```bash
# Install via npm
npm install -g jobtrail-cli

# Install via pip
pip install jobtrail-cli

# Install via cargo
cargo install jobtrail-cli

# Basic usage
jobtrail add "Google" "Software Engineer" --salary "150k" --location "Mountain View"  # Add job
jobtrail status "Google" applied                                          # Update status
jobtrail list --status applied                                            # List applied jobs
jobtrail interview "Google" 2024-01-15                                    # Schedule interview
jobtrail notes "Google" "Great company culture"                          # Add notes
jobtrail stats                                                            # Show statistics
jobtrail export --format csv                                              # Export data
jobtrail followup "Google" "Send thank you email" --date 2024-01-16      # Add follow-up
jobtrail report --days 30                                                 # Monthly report
```
...
Create a config file at `~/.jobtrail/config.json`:

```json
{
  "data_path": "~/.jobtrail/data.json",
