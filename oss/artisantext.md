# **ArtisanText** — *Simple Text Art Generator CLI*
...
ArtisanText is a simple CLI tool that lets you create text-based artwork, ASCII art, and simple graphics directly from your terminal. Perfect for developers and creators who want to generate visual content without complex design tools.
...
```bash
# Install via npm
npm install -g artisantext-cli

# Install via pip
pip install artisantext-cli

# Install via cargo
cargo install artisantext-cli

# Basic usage
artisantext text "Hello World"                    # Generate text art
artisantext ascii "cat"                          # Generate ASCII art
artisantext banner "Welcome" --style fancy       # Create banner
artisantext list-templates                       # Show available templates
artisantext export "Hello World" --format html   # Export as HTML
artisantext batch input.txt --output output/     # Process multiple files
```
...
Create a config file at `~/.artisantext/config.json`:
...
```json
{
  ...
  "output_format": "text",
  "color_enabled": true,
  "templates_path": "~/.artisantext/templates",
  "output_path": "./output"
}
```
