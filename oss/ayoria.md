# **Ayoria — Story Outline Generator CLI**
...
**Ayoria** is an open-source CLI tool that generates structured story outlines and basic prose from simple text inputs. It takes a story concept, title, or brief description and creates a complete story blueprint with characters, plot points, and chapter structure.
...
Ayoria is a simple CLI tool that:
...
ayoria generate "A detective solves a murder in a small town" --genre mystery

# Interactive mode
ayoria interactive

# Generate with specific output format
ayoria generate "Space adventure" --format json --output story.json

# Use custom template
ayoria generate "Romance story" --template romance --output romance_outline.md
```
...
Users can configure Ayoria using a `config.yaml` file:
...
For future LLM enhancement, Ayoria supports API key configuration:
...
# Or via config file
ayoria config set api_key "your-api-key-here"
```
...
**Ayoria** transforms story ideas into structured outlines, helping writers overcome creative blocks and develop compelling narratives quickly and efficiently.
