# **Nexida** — *Simple Campus Information CLI*
...
Nexida is a simple CLI tool that lets you manage campus information, track events, and organize academic resources directly from your terminal. Perfect for students, faculty, and campus administrators who want to organize campus information without complex systems.
...
```bash
# Install via npm
npm install -g nexida-cli

# Install via pip
pip install nexida-cli

# Install via cargo
cargo install nexida-cli

# Basic usage
nexida event add "Student Orientation" --date 2024-09-01 --location "Main Hall"     # Add event
nexida resource add "Computer Lab A" --type room --capacity 30                      # Add resource
nexida announce "Campus closed for holiday" --priority high --expires 2024-12-25   # Add announcement
nexida book "Computer Lab A" --start 2024-01-15 14:00 --end 16:00                  # Book resource
nexida events --category academic                                                   # List academic events
nexida resources --type room                                                       # List rooms
nexida calendar --month 1                                                          # Show January calendar
nexida export --format csv                                                         # Export data
nexida report --output campus_report.html                                          # Generate report
```
...
Create a config file at `~/.nexida/config.json`:

```json
{
  "data_path": "~/.nexida/data.json",
