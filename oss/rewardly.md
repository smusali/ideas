# **Rewardly** — *Simple Customer Loyalty CLI*
...
Rewardly is a simple CLI tool that lets you manage customer loyalty programs, track rewards, and analyze customer engagement directly from your terminal. Perfect for small businesses, developers, and anyone who wants to build customer loyalty without complex systems.
...
```bash
# Install via npm
npm install -g rewardly-cli

# Install via pip
pip install rewardly-cli

# Install via cargo
cargo install rewardly-cli

# Basic usage
rewardly customer add "John Doe" john@example.com              # Add customer
rewardly purchase "John Doe" 50.00 --category food            # Record purchase
rewardly points "John Doe"                                     # Check points
rewardly reward add "Free Coffee" 100 "Free coffee drink"     # Add reward
rewardly redeem "John Doe" "Free Coffee"                      # Redeem reward
rewardly customers --tier gold                                 # List customers by tier
rewardly analytics --days 30                                   # Show analytics
rewardly export --format csv                                   # Export data
rewardly report --customer "John Doe"                         # Customer report
```
...
Create a config file at `~/.rewardly/config.json`:

```json
{
  "data_path": "~/.rewardly/data.json",
  "points_per_dollar": 1,
  "default_tier": "bronze",
  "tier_thresholds": {
    "silver": 100,
