# **Chronara** — *Simple Time Tracking & Scheduling CLI*
...
Chronara is a simple CLI tool that lets you track time, manage schedules, and analyze productivity directly from your terminal. Perfect for freelancers, students, and anyone who wants to monitor their time usage without complex applications.
...
```bash
# Install via npm
npm install -g chronara-cli

# Install via pip
pip install chronara-cli

# Install via cargo
cargo install chronara-cli

# Basic usage
chronara start "Working on project"              # Start tracking
chronara stop                                    # Stop tracking
chronara status                                  # Show current status
chronara schedule add "Meeting" 14:00 15:00      # Add schedule
chronara report --days 7                         # Weekly report
chronara project add "Website Redesign"          # Add project
chronara analytics                               # Show insights
chronara export --format csv                     # Export data
```
...
Create a config file at `~/.chronara/config.json`:

```json
{
  "data_path": "~/.chronara/data.json",
