...
npm install -g elydra-cli
# or
pip install elydra-cli
# or
cargo install elydra-cli

# Process a statement
elydra process statement.pdf --output json

# Process multiple files
elydra batch ./statements/ --format csv

# Use AI assistance (requires API key)
elydra process statement.pdf --ai --api-key YOUR_KEY
```

### Configuration
```bash
# Set up API key for AI features
elydra config set api-key YOUR_LLM_API_KEY

# Configure output formats
elydra config set default-format json

# Set up custom categories
elydra config set categories ./my-categories.json
```
...
### Configuration File (~/.elydra/config.json)
```json
{
...
```bash
elydra process statement.pdf --output json --ai
```

### Batch Process Directory
```bash
elydra batch ./statements/ --format csv --output-dir ./processed
```

### Generate Analytics Report
```bash
elydra analyze ./processed/ --report spending-summary
```

### Export to Database
```bash
elydra export ./processed/ --format sqlite --db financial_data.db
```
...
1. **Install**: `npm install -g elydra-cli`
2. **Configure**: Set up your API key if using AI features
3. **Process**: Run `elydra process your-statement.pdf`
4. **Analyze**: Use `elydra analyze` for insights
5. **Contribute**: Check out the GitHub repository for ways to help

> **Elydra CLI: Transform your financial data into insights, one command at a time.**
