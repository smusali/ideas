# **LogInsight** — *Simple Log Analysis CLI*
...
LogInsight is a simple CLI tool that lets you analyze log files, search for patterns, and get insights about your application's behavior directly from your terminal. Perfect for developers and DevOps engineers who want to understand their logs without complex monitoring tools.
...
```bash
# Install via npm
npm install -g loginsight-cli

# Install via pip
pip install loginsight-cli

# Install via cargo
cargo install loginsight-cli

# Basic usage
loginsight analyze app.log                           # Analyze log file
loginsight query "Show me all errors from yesterday" # Natural language query
loginsight search "ERROR" --file app.log             # Search for errors
loginsight stats app.log                             # Show statistics
loginsight errors app.log --count                    # Count errors
loginsight export app.log --format csv               # Export analysis
loginsight pattern app.log --level error             # Find patterns
loginsight timeline app.log --hours 24               # Show timeline
loginsight report app.log --output report.html       # Generate report
```
...
Create a config file at `~/.loginsight/config.json`:

```json
{
  "data_path": "~/.loginsight/data",
  "default_format": "text",
  "max_file_size": "100MB",
  "export_format": "csv",
  "auto_parse": true,
