# **DataViz** — *Simple Data Visualization CLI*
...
DataViz is a simple CLI tool that lets you analyze CSV files, create charts and graphs, and get insights about patterns and trends directly from your terminal. Perfect for developers and analysts who want to understand and visualize their data without complex tools.
...
```bash
# Install via npm
npm install -g dataviz-cli

# Install via pip
pip install dataviz-cli

# Install via cargo
cargo install dataviz-cli

# Basic usage
dataviz analyze data.csv                           # Analyze CSV file
dataviz chart data.csv --type bar --column sales   # Create bar chart
dataviz summary data.csv                           # Show data summary
dataviz export data.csv --format html              # Export as HTML
dataviz filter data.csv --column age --gt 25       # Filter data
dataviz stats data.csv                             # Show statistics
dataviz report data.csv --output report.html       # Generate report
```
...
Create a config file at `~/.dataviz/config.json`:
...
- Share on Twitter/X with #opensource #dataviz #cli
