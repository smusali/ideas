# **SysPulse** — *Simple System Monitoring CLI*
...
SysPulse is a simple CLI tool that lets you monitor system performance, track metrics, and get insights about system health directly from your terminal. Perfect for developers and DevOps engineers who want to monitor their systems without complex tools.
...
```bash
# Install via npm
npm install -g syspulse-cli

# Install via pip
pip install syspulse-cli

# Install via cargo
cargo install syspulse-cli

# Basic usage
syspulse start --interval 5                              # Start monitoring
syspulse status                                          # Show current status
syspulse metrics --type cpu                              # Show CPU metrics
syspulse alert add cpu --threshold 80 --message "High CPU"  # Add alert
syspulse report --hours 24                               # Generate report
syspulse export --format csv                             # Export metrics
syspulse visualize --type memory                         # Show memory graph
syspulse stop                                            # Stop monitoring
syspulse config --interval 10                            # Configure interval
syspulse history --days 7                                # Show history
```
...
Create a config file at `~/.syspulse/config.json`:

```json
{
  "data_path": "~/.syspulse/data.json",
  "default_interval": 5,
  "export_format": "csv",
  "alert_enabled": true,
  "auto_start": false,
