# **FeedLoop** — *Simple Feedback Collection & Analysis CLI*
...
FeedLoop is a simple CLI tool that lets you collect feedback from users, organize it into meaningful categories, and analyze patterns and insights directly from your terminal. Perfect for product teams, researchers, and developers who want to understand user feedback without complex tools.
...
```bash
# Install via npm
npm install -g feedloop-cli

# Install via pip
pip install feedloop-cli

# Install via cargo
cargo install feedloop-cli

# Basic usage
feedloop form create "User Survey"              # Create feedback form
feedloop collect "User Survey" "Great app!"     # Collect feedback
feedloop categorize "User Survey" --auto        # Auto-categorize
feedloop analyze "User Survey" --sentiment      # Analyze sentiment
feedloop report "User Survey" --format html     # Generate report
feedloop export "User Survey" --format csv      # Export feedback
feedloop trends "User Survey" --days 30         # Show trends
feedloop stats "User Survey"                    # Show statistics
```
...
Create a config file at `~/.feedloop/config.json`:

```json
{
  "data_path": "~/.feedloop/data.json",
