# **Sentimently** — *Simple Feedback Analysis CLI*
...
Sentimently is a simple CLI tool that lets you analyze feedback, track sentiment, and understand user opinions directly from your terminal. Perfect for product managers, developers, and anyone who wants to understand user feedback without complex analysis tools.
...
```bash
# Install via npm
npm install -g sentimently-cli

# Install via pip
pip install sentimently-cli

# Install via cargo
cargo install sentimently-cli

# Basic usage
sentimently analyze feedback.csv                           # Analyze feedback file
sentimently sentiment "Great product!"                     # Analyze single feedback
sentimently trends --days 30                               # Show sentiment trends
sentimently categorize feedback.csv --output categories.json # Categorize feedback
sentimently summary feedback.csv --output summary.md       # Generate summary
sentimently export feedback.csv --format json              # Export analysis
sentimently keywords feedback.csv --top 10                 # Extract keywords
sentimently report feedback.csv --output report.html       # Generate report
sentimently compare feedback1.csv feedback2.csv            # Compare datasets
sentimently stats feedback.csv                             # Show statistics
```
...
Create a config file at `~/.sentimently/config.json`:

```json
{
  "data_path": "~/.sentimently/data.json",
